...
const Orders = () => {
  ...
  const navigateCancelOrderPage = (eventName) => {
...

{
  ...
  accessor: (row) => {
    // Destructure and handle defaults
    const {
      accommodationOndalindaPerDaysFeeAmount = 0,
      accommodationPerDaysPropertyOwnerAmount = 0,
      total_night_stay = 0,
    ...
    const fullHomeownerPayout = accommodationPerDaysPropertyOwnerAmount * total_night_stay;
    const fullOndalindaPayout = accommodationOndalindaPerDaysFeeAmount * total_night_stay;
    ...
    // Use BookAccommodationInfo if available, otherwise fallback to first AccommodationExtensions item
    const accommodationData = BookAccommodationInfo || AccommodationExtensions[0] || {};
    const Housing = accommodationData.Housing || {};
    const neighborhood = Housing.HousingNeighborhood?.name || "";
  ...
  disableSortBy: true,
    ...
    //   ${row.AccommodationExtensions?.Housing?.HousingNeighborhood?.name || ""}
    //   ${row.AccommodationExtensions?.Housing?.HousingNeighborhood?.name || ""}
    //   ${HomeownerPayout}
    //   ${OndalindaPayout}
    // `;
  ...
  const handleFormSubmitOld = async (event) => {
    setBasic(false);
  ...
  const handleFormSubmit = async (e) => {
    ...
    const eventNameFromURL = getEventNameFromURL(); // if needed
    const API_URL = `/api/v1/events`;
    ...
    try {
      ...
      // Append all form fields to FormData if they exist
      if (searchFormData.orderId)
        body.append("orderId", searchFormData.orderId.trim());
      if (searchFormData.eventName)
        body.append("eventName", searchFormData.eventName.trim());
      if (searchFormData.name) body.append("name", searchFormData.name.trim());
      if (searchFormData.lname)
        body.append("lname", searchFormData.lname.trim());
      if (searchFormData.email)
        body.append("email", searchFormData.email.trim());
      if (searchFormData.paymentOption)
        body.append("paymentOption", searchFormData.paymentOption);

      if (startDate) body.append("startDate", formatDate(startDate));
      if (endDate) body.append("endDate", formatDate(endDate));
  ...
  // kamal Resend All members send email ticket
  // let sendOrderIds = DATATABLE.map((e) => e.id);
  ...
  const handleDonationOrders = (row) => {
    const encodedName = encodeURIComponent(eventName);
    router.push({
      pathname: "/admin/orders/donation",
      query: {
        event: encodedName,
      },
    });
