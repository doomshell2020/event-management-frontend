...
const InvitationsTable = () => {
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <span className="ms-auto invit-mbl tx-14">
          ...
          {row.original.Status == 2 && row.original.User && row.original.User.MyOrders && (
            (() => {
              const orders = row.original.User.MyOrders;
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <>
          {["Las Rosadas", "Cuixmala", "Las Alamandas"].includes(row.original.accommodation_status) ? (
            ...
            row.original.accommodation_status !== "Booked" && (
              <span className="ms-auto invit-mbl tx-14">
                ...
                {row.original.Status === 2 &&
                  (row.original.accommodation_status == "Preference Submitted" ||
                    row.original.accommodation_status == "Property Offered") && (
          ...
          {(row.original.accommodation_status === 'Booked' || row.original.Status === 2) && (() => {
            const orders = row.original.User?.MyOrders || [];
            ...
            // Combine main bookings and extensions
            const allBookings = [];

            orders.forEach(order => {
              const bookInfo = order.BookAccommodationInfo;
              const extensions = order.AccommodationExtensions || [];

              // If there's a main booking
              if (bookInfo) {
                const housing = bookInfo.Housing;
                const housingNameUrl = housing?.Name?.trim().replace(/ /g, "+") || "";
                const checkIn = formatDate(bookInfo.check_in_date);
                const checkOut = formatDate(bookInfo.check_out_date);
                const year = new Date(bookInfo.check_in_date).getFullYear();
                const dateRange = `${checkIn} - ${checkOut}, ${year}`;

                allBookings.push({
                  id: `${order.id}-main`,
                  name: housing?.Name || 'N/A',
                  neighborhood: housing?.HousingNeighborhood?.name || '',
                  url: `/housing/${housingNameUrl}`,
                  dateRange,
                  isExtended: false
                });
              }
              ...
              extensions.forEach(ext => {
                const housing = ext.Housing;
                const housingNameUrl = housing?.Name?.trim().replace(/ /g, "+") || "";
                const checkIn = formatDate(ext.check_in_date);
                const checkOut = formatDate(ext.check_out_date);
                const year = new Date(ext.check_in_date).getFullYear();
                const dateRange = `${checkIn} - ${checkOut}, ${year}`;

                allBookings.push({
                  id: `${order.id}-ext-${ext.id}`,
                  name: housing?.Name || 'N/A',
                  neighborhood: housing?.HousingNeighborhood?.name || '',
                  url: `/housing/${housingNameUrl}`,
                  dateRange,
            ...
            return allBookings.length > 0 ? (
              allBookings.map((booking, index) => (
                <div key={booking.id} style={{ fontSize: "12px", borderBottom: "6px" }}>
                  <Link
                    title="View property details"
                    target="_blank"
                    href={booking.url}
                  ...
                  {booking.isExtended && (
                    <span style={{ color: 'green', fontSize: '11px', marginLeft: '6px' }}>
                      (Extended)
                    </span>
                  )}
                  <br />
          ...
          {/* {(row.original.accommodation_status === 'Booked' || row.original.Status === 2) && (() => {
            ...
            // Filter orders with BookAccommodationInfo
            const bookedAccommodations = orders
              .filter(order => order.BookAccommodationInfo)
              .map((order, index, arr) => {
                // .map(order => {
                ...
                const dateRange = `${format(check_in_date)} - ${format(check_out_date)}, ${new Date(check_in_date).getFullYear()}`;
                const housingNameUrl = Housing?.Name ? Housing.Name.trim().replace(/ /g, "+") : "";
                return (

                  <div key={order.id} style={{ fontSize: "12px", borderBottom: "6px" }}>
                    <Link
                      title="View property details"
                      target="_blank"
                    ...
                    >
                      <strong>
                        {Housing?.Name || "N/A"}
                        {Housing?.HousingNeighborhood?.name? `, ${Housing.HousingNeighborhood.name}`
                          : ""}
                      </strong>
                    </Link>
                    <br />
            ...
            return bookedAccommodations.length > 0 ? bookedAccommodations : null;
          })()} */}
  ...
  useEffect(() => {
    ...
    if (attendees && attendees.length > 0) {
      ...
      axios.post("/api/v1/invitationevents", { EventID: id }).then((res) => {
        // console.log("🚀 ~ .then ~ value:", res.data)
  ...
  const getStatusBasedData = async () => {
    ...
    await axios
      ...
      .then((res) => {
        setIsLoading(false);
        setPageSize(res.data.searchResults.length);
        SetDATATABLE(res.data.searchResults);
                ...
                <Card.Header className="">
                  <div className="d-flex justify-content-end align-items-center">
                    ...
                    {attendees ? (
                      ...
                      >
                        Total Attendees: {DATATABLE && DATATABLE.length}
                      </span>
                    ) : (
                      <></>
                    )}
